...
public class Puzzle7_2 extends AbstractPuzzle {

	private static final BiFunction<Long, Long, Long> CONCATENATION = (t, u) -> Long
			.valueOf(t.toString() + u.toString());
		...
		public boolean testEquation() {
			...
			for (int i = 0; i < factors.size() - 1; i++) {
				if (i == 0) {
					...
					List<BiFunction<Long, Long, Long>> concatStartOperators = new LinkedList<>();
					concatStartOperators.add(CONCATENATION);
					operatorsList.add(sumStartOperators);
					operatorsList.add(mulStartOperators);
					operatorsList.add(concatStartOperators);
				} else {
					...
					for (List<BiFunction<Long, Long, Long>> operators : operatorsList) {
						...
						List<BiFunction<Long, Long, Long>> duplicatedForMulOperators = new LinkedList<>(operators);
						List<BiFunction<Long, Long, Long>> duplicatedForConcatOperators = new LinkedList<>(operators);
						duplicatedForMulOperators.add(Math::multiplyExact);
						duplicatedForConcatOperators.add(CONCATENATION);
						newOperationLists.add(duplicatedForMulOperators);
						newOperationLists.add(duplicatedForConcatOperators);
